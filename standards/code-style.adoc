...
be done if the logic is unmistakable and doing so increases readability of the method.
You may _consider_ omitting braces if the situation meets the following conditions:

. the conditional and the branch each contain at most a single predicate
. it fits on a single line

Remember that no one is going to complain about Braces being present.
