...
Braces are not required for `when` branches and `if` statement bodies if they meet ALL
the following conditions:
- there are no `else if`/`else` branches
- the conditional and the branch contain at most a single predicate
